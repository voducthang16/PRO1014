<?php
    class homeModels extends database {
        function getProducts() {
            ...
            quantity ON products.id = quantity.product_id WHERE products.category_id != 2 ORDER BY products.id DESC LIMIT 8";
        ...
        function getHoodiesProducts() {
            ...
            WHERE products_type.status = 1 AND category_id = 2 GROUP BY products_type.price_origin, products_type.price_sale ORDER BY id DESC LIMIT 6";
            $result = $this->connect->prepare($query);
            $result->execute();
            return $result->fetchAll();
        }
