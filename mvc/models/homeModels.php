<?php
    class homeModels extends database {
        function getProducts() {
            $query = "SELECT products.id, category_id, name, slug, thumbnail, products_type.price_origin, products_type.price_sale FROM `products` 
            INNER JOIN products_type ON products.id = products_type.product_id 
            WHERE status = 1 and category_id != 2 GROUP BY products_type.price_origin, products_type.price_sale ORDER BY id DESC LIMIT 8";
