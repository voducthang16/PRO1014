<?php
    class productModels extends database {
        ...
        function getProductAttribute($attribute, $id) {
            ...
            IN (SELECT products_type.id FROM products_type WHERE products_type.product_id = $id AND products_type.status = 1) AND products_attributes.name LIKE '%$attribute%'";
            $result = $this->connect->prepare($query);
            $result->execute();
            return $result->fetchAll();
        }
